...
//  R = U # V
{
    ...
    for (i = 0; i < P; i++)
        for (j = 0; j < Q1; j++, pRij++)
        {
            ...
            pUik = pU + Q*i;
            pVkj = pV + j;
            for (k = 0; k < Q; k++, pUik++, pVkj+=Q1)
                Val +=  (double) *pUik * (double) *pVkj;
            *pRij = Val;
        }
//    for (i = 0; i < Q1; i++)
//        for (j = 0; j < P; j++)
...
// MATRIX inversion:efficient if number of lines >= number of columns
// InvMat = Mat^-1
// Mat: IN = matrix with P = Mat.ny  and Q = Mat.nx   (P >= Q)
//  
{
...
// If wmax is the largest eigenvalue, all eigenvalue below minT * wmax will be set to 0
{
    ...
    double *ptrMat = Mat.buffer();
    for(i=0; i < P; i++) { // number of lines
        for(j=0; j < Q; j++, ptrMat++) // number of columns
            a[i+1][j+1]= *ptrMat;
    }
