...
void io_strcpy_prefix(char* Prefix, const char* Filename);
...
Bool std_inout(const char* Filename);
...
int cfitstio_read_celestial_coord(const char *File_Name, fitsstruct *FitsHeader);
...
void io_read_ima_int  (const char* File_Name, Iint &, fitsstruct *hd = NULL, Bool NoBscale = False);
void io_write_ima_int (const char *, Iint &, fitsstruct *hd = NULL);
void io_read_ima_float  (const char *, Ifloat &, fitsstruct *hd = NULL);
void io_write_ima_float (const char *, Ifloat &, fitsstruct *hd = NULL, Bool NormData=False);
void io_read_ima_complex_f  (const char *, Icomplex_f &);
void io_write_ima_complex_f (const char *, Icomplex_f &);

char *fitsname(const char* NameStep);
FILE *fits_file_des_in(const char *fname);
FILE *fits_file_des_out(const char *fname);
void fits_read_header(const char *File_Name, fitsstruct *Header);
void fits_read_fltarr(const char *File_Name, fltarray &Mat);
void fits_read_fltarr(const char *File_Name, fltarray &Mat, fitsstruct * FitsHeader);
void fits_read_fltarr(const char* File_Name, fltarray &Mat, fitsstruct * FitsHeader,
                      int openflag);



void fits_read_cfarr(const char *File_Name, cfarray &Mat);
void fits_read_cfarr(const char *File_Name, cfarray &Mat, fitsstruct * FitsHeader);
void fits_read_cfarr(const char *File_Name, cfarray & DataCF, fitsstruct *Header,
					  int openflag);



void fits_read_block(const char *filename, Ifloat & Image, int Indi, int Indj, Bool NoBscale=False);
void fits_read_block(const char *filename, Iint & Image, int Indi, int Indj, Bool NoBscale=False);
void fits_write_block(const char *filename, Ifloat & Image, int Indi, int Indj,
                      Bool NoBscale=False);
void fits_write_block(const char *filename, Iint & Image, int Indi, int Indj,
                      Bool NoBscale=False);
void fits_read_float(const char *File_Name,Ifloat & Image,fitsstruct *Header);
void fits_read_int(const char *File_Name,Iint & Image,
                     fitsstruct *Header, Bool NoBscale=False);

void fits_write_float(const char *File_Name,Ifloat & Image,fitsstruct *Header);
void fits_write_int(const char *File_Name, Iint & Image,fitsstruct *Header);
void fits_write_header(const char *File_Name, fitsstruct *Header);

void fits_write_fltarr(const char *File_Name, fltarray &Mat);
void fits_write_fltarr(const char *File_Name, fltarray &Mat, fitsstruct *FitsHeader);
void fits_read_dblarr(const char *File_Name, dblarray & Data);
void fits_write_dblarr(const char *File_Name, dblarray & Data);
void fits_write_intarr(const char * FileName, intarray & Tab);
void fits_read_intarr(const char * FileName, intarray & Tab);

void fits_read_cfarr2d(const char * filename, cfarray &Mat);
void fits_write_cfarr2d(const char * filename, cfarray &Mat);
...
int fitsread(const char *fitsbuf, char *keyword, void *ptr,
...
void fits_read_block(const char *filename, fltarray & Image, int Indi, int Indj, Bool NoBscale=False);
void fits_read_block(const char *filename, intarray & Image, int Indi, int Indj, Bool NoBscale=False);
void fits_write_block(const char *filename, intarray & Image, int Indi, int Indj,
                      Bool NoBscale=False);
void fits_write_block(const char *filename, fltarray & Image, int Indi, int Indj,
                      Bool NoBscale=False);
...
class io_image {
        ...
        public:
           ...
           type_data Type;
           io_image() {};
           virtual void read_ima_int  (const char *, Iint &){}
           virtual void write_ima_int (const char *, Iint &){}
           virtual void read_ima_float  (const char *, Ifloat &){}
           virtual void write_ima_float (const char *, Ifloat &){}
           virtual void read_ima_complex_f  (const char *, Icomplex_f &){}
           virtual void write_ima_complex_f (const char *, Icomplex_f &){}
           virtual ~io_image(){};
             };

class io_disp: public virtual io_image {
          void read_type(const char *File_Name);
          void read_size (const char *);
          void read_ima (const char *);
          void write_ima (const char *);
         public:
          io_disp() {Nl_io = 0; Nc_io = 0; Type = UNKNOWN;}
          ~io_disp(){};
          void read_ima_int  (const char *, Iint &);
          void write_ima_int (const char *, Iint &);
          void read_ima_float  (const char *, Ifloat &);
          void write_ima_float (const char *,  Ifloat &);
          void read_ima_complex_f  (const char *, Icomplex_f &);
          void write_ima_complex_f (const char *, Icomplex_f &);
      void read_info_ima(const char *File_Name, int &Nl, int &Nc,
                            type_data & TypeDat);
     void read_block_ima(const char *File_Name,Ifloat & Image,int Indi, int Indj);
     void read_block_ima(const char *File_Name,Iint & Image,int Indi, int Indj);
     void write_block_ima(const char *File_Name,Ifloat & Image,int Indi, int Indj);
     void write_block_ima(const char *File_Name,Iint & Image,int Indi, int Indj);
	 
         void create_file (const char *File_Step_Name);
              };
...
class io_midas: public virtual io_image {
         public:
          ...
          ~io_midas();
          void read_ima_int  (const char *, Iint &);
          void write_ima_int (const char *,const Iint &);
          void read_ima_float  (const char *, Ifloat &);
          void write_ima_float (const char *,const Ifloat &);
          void read_ima_complex_f  (const char *, Icomplex_f &);
          void write_ima_complex_f (const char *,const Icomplex_f &);
      void read_info (const char *File_Name, IOInfoData &InfoDat);
             };
#endif

#include "IM_IOCol.h"

void readgif(const char *fname, Ifloat &Data);
void writegif(const char *fname, Ifloat & Data, Bool NormData=False);
void readgif(const char *fname, Iint & Data);
void readgif(const char *fname, PICINFO &pinfo);
void writegif(const char *fname, Iint & Data, Bool NormData=False);
int readgif(FILE *fp, PICINFO *pinfo);
int writegif(FILE *fp, PICINFO pinfo);
void writegif(const char *fname, PICINFO & pinfo);

void writejpeg(const char *fname, Ifloat & Data, Bool NormData=False);
void writejpeg(const char *fname, Iint & Data, Bool NormData=False);

// gif block access routines
void readhdgif(const char *filename, int &Nl, int &Nc);
void readhdgif(const char *File_Name, int &Nl, int &Nc, PICINFO * & GifIma);
...
void closegif(const char *FileName); // write the byte array to the disk
void closegif(const char *File_Name, PICINFO *GifIma);
...
FILE *pgm_file_des_in(const char *fname);
FILE *pgm_file_des_out(const char *fname);
void readpgm (const char *filename, int &Nl, int &Nc, unsigned char *  &buffer);
void readpgm(const char *fname, Iint &Data);
void readpgm(const char *fname, Ifloat &Data);
void writepgm (const char *filename, int Nl, int Nc, unsigned char *buffer);
void writepgm(const char *fname, Iint &Data, Bool NormData=False);
void writepgm(const char *fname, Ifloat &Data, Bool NormData=False);
// block access
...
void writehdpgm(const char *filename, int Nl, int Nc);
void write_block_pgm (const char *filename, int Nl, int Nc,
