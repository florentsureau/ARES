...
void fits_read_fltarr(const char *File_Name, fltarray & Data)
{
...
void fits_read_fltarr(const char *File_Name, fltarray & Data, fitsstruct *FitsHeader)
{
   fits_read_fltarr(File_Name, Data, FitsHeader, 1);
}
...
void fits_read_fltarr(const char *File_Name, fltarray & Data, fitsstruct *Header,
 int openflag)
{
...
void fits_read_block(const char *filename, intarray & Image, int Indi, int Indj, Bool NoBscale)
{
    // for fits
    fitsfile *fptr;           /* pointer to the FITS file */
...
void fits_read_block(const char *filename, fltarray & Image, int Indi, int Indj, Bool NoBscale)
{
    // for fits
    fitsfile *fptr;           /* pointer to the FITS file */
    int status=0, hdutype ;
...
void fits_write_intarr(const char * FileName, intarray & Tab)
{
	...
	for(int j=0;j<MAX(1,Tab.ny());j++)
	for(int k=0;k<MAX(1,Tab.nz());k++)
...
void fits_read_cfarr(const char *File_Name, cfarray & Data)
{
   fitsstruct FitsHeader;
...
void fits_read_cfarr(const char *File_Name, cfarray & Data, fitsstruct *FitsHeader)
{
   fits_read_cfarr(File_Name, Data, FitsHeader, 1);
}
...
void fits_read_cfarr2d(const char * filename, cfarray &Mat)
{
    ...
    for(int j=0;j<Mat.ny();j++) 
    for(int i=0;i<Mat.nx();i++)
        Mat(i,j) = std::complex<float>(Data(i,j,0),Data(i,j,1));
}
