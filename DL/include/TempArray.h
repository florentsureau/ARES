...
*/
    ...
    void display (int pi_NbElem=0, string Name = "") const;
    void display (int nx, int ny, int nz = 0, string Name = "", size_t precision = 7) const;
    void display (string Name = "", size_t precision = 7) const;
    void rampgen ();
...
#ifdef _USEMEM
        MemMg_free<PARAM_TYPE>(po_Buffer);
#endif
    } else {
...
void to_array<PARAM_TYPE,ARRAY_TYPE>::alloc (int pi_Nx, int pi_Ny, int pi_Nz, const char* Name) {
...
#ifdef _USEMEM
        po_Buffer = MemMg_alloc<PARAM_TYPE> (static_cast<size_t>(i_NbElem));
    ...
    if (ARRAY_TYPE==true) {
        ...
        pto_TabNaxis[1] = pi_Ny;
        is_ima = false;
    } else {
        pto_TabNaxis[0] = pi_Ny;
...
{// doesnt work well, treated as an image
    ...
    pto_TabNaxis[1] = _Ny;
    pto_TabNaxis[2] = _Nz;
    if (_Nz != 0) i_NbElem = _Nx*_Ny*_Nz;
    else if (_Ny != 0) i_NbElem = _Ny*_Nx;
    else i_NbElem = _Nx;
    i_NbAxis = (_Nx != 0) ? 1 : 0;
    i_NbAxis = (_Ny != 0) ? 2 : i_NbAxis;
    i_NbAxis = (_Nz != 0) ? 3 : i_NbAxis;
    is_ima = false;
}
...
#ifdef _USEMEM
                e_UseClassMemAlloc = true;
                po_Buffer = MemMg_alloc<PARAM_TYPE> (static_cast<size_t>(ai_Inter));
#else
                e_UseClassMemAlloc = false;
...
//------------------------------------------------------------------------------
//! input x and y are horizontal and vertical coordinates
//! array->nx is the major coordinate (x if ARRAY_TYPE, y if not)
//! array->ny is the minor coordinate (y if ARRAY_TYPE, x if not)
template <class PARAM_TYPE, bool ARRAY_TYPE>
...
//! Prints the array structural information
template <class PARAM_TYPE, bool ARRAY_TYPE>
void to_array<PARAM_TYPE,ARRAY_TYPE>::printInfo(string Name) const {
    if (Name=="") cout << "   : ";
    else cout << Name << " : ";
    cout << "to_array<" << typeid(PARAM_TYPE).name() << "," << ARRAY_TYPE << "> pointer=" << e_GetBuffer << ", (" << i_NbAxis << "D) = "
         << nx() << "x" << ny() << "x" << nz() << " = " << i_NbElem << endl;
}
...
template <class PARAM_TYPE, bool ARRAY_TYPE>
// !!!!! PARAM_TYPE must accept operator << !!!!!!!!!!!
void to_array<PARAM_TYPE,ARRAY_TYPE>::display (int pi_NbElem, string Name) const {
    if (pi_NbElem == 0) {
...
template <class PARAM_TYPE, bool ARRAY_TYPE>
void to_array<PARAM_TYPE,ARRAY_TYPE>::display(int Nx, int Ny, int Nz, string Name, size_t precision) const {
    printInfo(Name);
    if(Nx==0 || Nx > nx()) Nx = MAX(nx(), 1);
    if(Ny==0 || Ny > ny()) Ny = MAX(ny(), 1);
    if(Nz==0 || Nz > nz()) Nz = MAX(nz(), 1);
    for (int z=0; z < Nz; z++) {
        for (int y=0; y < Ny; y++) {
            cout << "   ";
            for (int x=0; x < Nx; x++)
                if (ARRAY_TYPE==true) {
                    cout << std::setprecision(precision) << std::setw(precision+5) << po_Buffer[z*pto_TabNaxis[0]*pto_TabNaxis[1] + y*pto_TabNaxis[0] + x] << " ";
                } else {
                    cout << std::setprecision(precision) << std::setw(precision+5) << po_Buffer[z*pto_TabNaxis[0]*pto_TabNaxis[1] + x*pto_TabNaxis[0] + y] << " ";
                }
            cout << endl;
        }
        cout << endl;
...
template <class PARAM_TYPE, bool ARRAY_TYPE>
void to_array<PARAM_TYPE,ARRAY_TYPE>::display(string Name, size_t precision) const {
    display(0, 0, 0, Name, precision);
}
